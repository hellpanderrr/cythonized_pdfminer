...
from pdfminer2.utils import INF,  get_bound, uniq, csort, fsplit, Plane
from pdfminer2.utils import bbox2str, matrix2str, apply_matrix_pt, mult_matrix
from pdfminer2.pdftypes import PDFStream
...
##  LAParams
##
cdef class LAParams(object):
    cdef public float line_overlap,char_margin,line_margin,word_margin,boxes_flow
    cdef public bint detect_vertical,all_texts
    def __init__(self,
                 line_overlap=0.5,
                 char_margin=2.0,
                 line_margin=0.5,
...
##  LTComponent
##
cdef class LTComponent(LTItem):
    cdef public float x0,x1,y0,y1,width,height
    cdef public tuple bbox
...
##  LTCurve
##
cdef class LTCurve(LTComponent):
...
##  LTImage
##
cdef class LTImage(LTComponent):
    
    cdef public unicode name
    cdef public object stream,srcsize,imagemask,bits,colorspace
...
##  LTChar
##
cdef class LTChar(LTComponent):
    cdef public object _text, adv, matrix, font, fontsize, scaling, rise, text, textwidth, textdisp, fontname, size,upright 
...
##  LTContainer
##
cdef class LTContainer(LTComponent):
...
##
cdef class LTExpandableContainer(LTContainer):
...
##  LTTextContainer
##
cdef class LTTextContainer(LTExpandableContainer):
    ...
    def get_text(self):
        return ''.join(obj.get_text() for obj in self if isinstance(obj, LTComponent) or isinstance(obj, LTAnno) or isinstance(obj, LTChar))
...
##  LTTextLine
##
cdef class LTTextLine(LTTextContainer):
...
cdef class LTTextLineHorizontal(LTTextLine):

    cdef public float word_margin,_x1,d
...
cdef class LTTextLineVertical(LTTextLine):
    cdef public int _y0
    def __init__(self, word_margin):
        LTTextLine.__init__(self, word_margin)
        self._y0 = -INF
...
##
cdef class LTTextBox(LTTextContainer):
...
cdef class LTTextBoxHorizontal(LTTextBox):
...
cdef class LTTextBoxVertical(LTTextBox):
...
##  LTTextGroup
##
cdef class LTTextGroup(LTTextContainer):
...
cdef float dist(object obj1,object obj2):
    ...
    x0 = float_min(obj1.x0, obj2.x0)
    y0 = float_min(obj1.y0, obj2.y0)
    x1 = float_max(obj1.x1, obj2.x1)
    y1 = float_max(obj1.y1, obj2.y1)
    return ((x1-x0)*(y1-y0) - obj1.width*obj1.height - obj2.width*obj2.height)

cdef isany(plane, obj1, obj2):
...
##  LTLayoutContainer
##
cdef class LTLayoutContainer(LTContainer):
    ...
    cdef group_objects(self, laparams, objs):
        obj0 = None
        line = None
        
        #print objs[0]
        #dump(objs,open('objects','wb'))
        ret = []
        for obj1 in objs:
            if obj0 is not None:
    ...
    cdef group_textboxes(self, laparams, boxes):
        assert boxes
        ...
        while dists:
            ...
            if c == 0 and isany(plane, obj1, obj2):
...
##  LTFigure
##
cdef class LTFigure(LTLayoutContainer):
    cdef public unicode name 
    cdef public tuple matrix
...
# ##  LTPage
# ##
cdef class LTPage(LTLayoutContainer):
    cdef public float pageid
    cdef public float rotate
